...
export default function PaqueteModal({ open, onClose, onComplete }: {
  open: boolean,
  onClose: () => void,
  onComplete: (data: { personas: number, inicio: Date, fin: Date }) => void
}) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onComplete({ personas, inicio: range.from, fin: range.to });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex items-center justify-center">
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
        <h2 className="text-xl font-bold mb-4">Datos para Paquete/Excursión</h2>
